...
export default function StructureRedirectPage() {
  ...
  useEffect(() => {
    ...
    if (projectId) {
      router.replace(`/project/${projectId}/structure`);
    } else {
      router.replace("/");
    }
  ...
  return (
    <div className="w-screen h-screen flex flex-col items-center justify-center bg-[#0A0B0E] text-[#9E9C98]">
      ...
      <span className="text-xs font-mono tracking-widest uppercase">Directing to Structure View...</span>
    </div>
  );
}

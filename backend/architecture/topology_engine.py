...
def generate_architectural_schemes(
    ...
    # Scheme 4: Vastu-Optimized Directional Scheme (when Vastu is enabled)
    if vastu_compliant:
        from vastu.vastu_engine import get_vastu_topological_scheme_placements
        s_vastu = ArchitecturalScheme(
            scheme_id="scheme_vastu_optimized",
            name="Vastu Shastra Harmonic Scheme",
            description="Orientation-aware directional zoning: Master bedroom in Southwest (Nairrutya), Kitchen in Southeast (Agneya), Puja/Living in Northeast/East."
        )
        s_vastu.circulation_type = "central"
        s_vastu.priority_adjacencies = [["living_room", "entry_foyer"], ["dining", "kitchen"]]
        s_vastu.priority_separations = [["master_bedroom", "kitchen"], ["bathroom", "pooja"]]
        s_vastu.zone_placements = get_vastu_topological_scheme_placements(
            road_side=road,
            rooms=rooms,
            plot_width=site.plot_width,
            plot_length=site.plot_length
        )
        schemes.insert(0, s_vastu)

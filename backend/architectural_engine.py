...
from models import (
    ...
    Stair, CirculationNetwork
)
from site_planner import plan_site
from zoning_graph import ZONE_MAP, PRIVACY_MAP
from topology_engine import generate_architectural_schemes, ArchitecturalScheme
from spatial_solver import solve_spatial_layout, SolverCandidate
...
def generate_architectural_house_layout(
    plot_width: float = 42.0,
    plot_length: float = 36.0,
    num_floors: int = 1,
    bedrooms: int = 3,
    bathrooms: float = 2.0,
    attached_bathroom_count: Optional[int] = None,
    ...
    # 2. Structure architectural requirements via Pydantic model
    ...
    # 3. Formulate 2-3 Architectural Concept Strategies via Groq Active Reasoning Layer
    ...
    for floor_idx in range(1, num_floors + 1):
        ...
        # Multi-floor staircase alignment: pin upper staircase to ground floor core
        ...
        for s in schemes:
            ...
            if not candidate or not candidate.is_valid:
                # If pinned solve is infeasible on a constrained layout, fallback unpinned
                if pinned_rooms:
                    candidate = solve_spatial_layout(
                        floor_rooms, site, s,
                        variant_seed=variant_seed
            ...
            if candidate and candidate.is_valid:
                ...
                # Deduplicate walls and derive doors + windows
                walls, doors, windows = generate_wall_network_and_openings(candidate.rooms, site)
        ...
        if not solved_candidates:
            # Fallback: create emergency graceful candidate if plot envelope was overly constrained
            # Ensure application never crashes even on extreme edge cases
            ...
            if not candidate:
                # Return informative impossible requirements message
        ...
        # Have Groq Architectural Critic evaluate candidates and pick winner
        if floor_idx == 1:
            groq_critique_obj = critique_architectural_candidates_with_groq(
                candidate_summaries=candidate_summaries,
                user_prompt=user_prompt or f"{bedrooms} bedroom {style} home"
            )
            groq_critique_data = groq_critique_obj.model_dump() if hasattr(groq_critique_obj, "model_dump") else groq_critique_obj
            winner_id = groq_critique_data.get("selected_candidate", candidate_summaries[0]["scheme_id"])
        else:
            winner_id = candidate_summaries[0]["scheme_id"]

        # Select champion candidate

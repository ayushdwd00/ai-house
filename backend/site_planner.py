...
from models import Site, Setbacks, ParkingSpace, Rect, Point2D


def compute_setbacks(plot_width: float, plot_length: float, road_side: str) -> Setbacks:
    """
    Computes residential architectural setbacks based on plot area and frontage bylaws.
    Ensures adequate natural light, ventilation buffers, and fire/access clearances.
    """
    ...
    return Setbacks(front=front, rear=rear, left=left, right=right)
...
def plan_site(
    ...
    custom_setbacks: Optional[Setbacks] = None
) -> Site:
    """
    ...
    """
    setbacks = custom_setbacks or compute_setbacks(plot_width, plot_length, road_side)
